...
public class CertificateGenerator {
    ...
    private void addExtensions(X509v3CertificateBuilder certGen, List<String> extensions) throws CertIOException {
        if (extensions.contains(CertificateExtensions.AUTHORITY_KEY_IDENTIFIER.name())) {
            certGen.addExtension(Extension.authorityKeyIdentifier, false, new AuthorityInformationAccess(
                    AccessDescription.id_ad_caIssuers,
                    null)
            );
        } else if (extensions.contains(CertificateExtensions.BASIC_CONSTRAINTS.name())) {
            certGen.addExtension(Extension.basicConstraints, false, new BasicConstraints(false));
        } else if (extensions.contains(CertificateExtensions.KEY_USAGE.name())) {
            certGen.addExtension(Extension.keyUsage, false, new KeyUsage(KeyUsage.digitalSignature | KeyUsage.keyEncipherment));
        } else if (extensions.contains(CertificateExtensions.EXTENDED_KEY_USAGE.name())) {
